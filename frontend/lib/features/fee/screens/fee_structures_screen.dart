...
class _FeeStructuresScreenState extends State<FeeStructuresScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      setState(() { _structures = d; _loading = false; });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Fee Structures', style: TextStyle(color: AppColors.darkOlive, fontWeight: FontWeight.w700)),
        actions: [
          IconButton(
  ...
  Future<void> _deleteStructure(FeeStructureModel s) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title: const Text('Delete Structure', style: TextStyle(color: AppColors.darkOlive)),
        content: Text('Delete "${s.name}"? If it has active records it will be deactivated instead.'),
        actions: [
...
class _StructureTile extends StatelessWidget {
  ...
  const _StructureTile({required this.structure, required this.onEdit, required this.onDelete});
    ...
    return Container(
      ...
      child: ListTile(
        ...
        leading: Container(
          ...
          child: const Icon(Icons.receipt_rounded, color: AppColors.darkOlive, size: 20),
        ),
        title: Text(structure.name, style: const TextStyle(fontWeight: FontWeight.w700, color: AppColors.darkOlive)),
        subtitle: Column(
          ...
          children: [
            ...
            if (structure.lateFinePerDay > 0)
              Text('Late fine: ₹${structure.lateFinePerDay}/day', style: const TextStyle(color: Color(0xFFC62828), fontSize: 11)),
            Text('${structure.assignmentCount} student${structure.assignmentCount == 1 ? '' : 's'}', style: const TextStyle(color: AppColors.mutedOlive, fontSize: 11)),
          ],
        ),
        trailing: PopupMenuButton<String>(
          icon: const Icon(Icons.more_vert_rounded, color: AppColors.mutedOlive),
...
class _StructureFormSheet extends StatefulWidget {
  ...
  final Future<void> Function(String name, String? desc, double amount, String cycle, double fine) onSubmit;
...
class _StructureFormSheetState extends State<_StructureFormSheet> {
  ...
  static const _cycles = ['ONCE', 'MONTHLY', 'QUARTERLY', 'HALF_YEARLY', 'YEARLY'];
  static const _cycleLabels = {'ONCE': 'One-time', 'MONTHLY': 'Monthly', 'QUARTERLY': 'Quarterly', 'HALF_YEARLY': 'Half-yearly', 'YEARLY': 'Yearly'};
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Text(widget.existing != null ? 'Edit Structure' : 'New Fee Structure', style: Theme.of(context).textTheme.titleLarge),
            const SizedBox(height: 16),
            TextField(controller: _nameCtrl, decoration: const InputDecoration(labelText: 'Name *', hintText: 'e.g. Monthly Tuition')),
            ...
            const Text('Billing Cycle', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.darkOlive, fontSize: 13)),
              ...
              children: _cycles.map((c) {
                ...
                return GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Text(_cycleLabels[c] ?? c, style: TextStyle(color: sel ? AppColors.cream : AppColors.darkOlive, fontSize: 12, fontWeight: FontWeight.w600)),
                  ),
                );
              }).toList(),
            ),
            ...
            TextField(
              ...
              decoration: const InputDecoration(labelText: 'Late fine per day (₹)', hintText: '0'),
            ),
  ...
  Future<void> _submit() async {
    ...
    if (name.isEmpty || amount == null || amount <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Name and a valid amount are required')));
      return;
    }
    setState(() => _submitting = true);
    try {
      await widget.onSubmit(name, _descCtrl.text.trim().isEmpty ? null : _descCtrl.text.trim(), amount, _cycle, double.tryParse(_fineCtrl.text) ?? 0);
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          const Icon(Icons.receipt_long_outlined, size: 52, color: AppColors.mutedOlive),
          const SizedBox(height: 12),
          const Text('No fee structures yet', style: TextStyle(color: AppColors.mutedOlive, fontWeight: FontWeight.w500)),
          const SizedBox(height: 6),
          const Text('Create templates like "Monthly Tuition"\nto assign to students.', textAlign: TextAlign.center, style: TextStyle(color: AppColors.mutedOlive, fontSize: 13)),
          const SizedBox(height: 20),
          FilledButton.icon(onPressed: onCreate, icon: const Icon(Icons.add_rounded), label: const Text('Create Structure')),
        ],
      ),
    );
  }
